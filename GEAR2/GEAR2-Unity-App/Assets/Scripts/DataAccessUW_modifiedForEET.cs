...
namespace ARETT
{
	...
	/// </summary>
	public class DataAccessUWP_MODIFIED : IDataAccess
	{
		/// <summary>
		...
		public DataAccessUWP_MODIFIED(int fetchDataSleepMs, ConcurrentQueue<GazeAPIData> dataQueue)
		{
			// Keep the configuration
			this.fetchDataSleepMs = fetchDataSleepMs;
			this.dataQueue = dataQueue;

			extendedEyeGazeDataProvider = new ExtendedEyeGazeDataProvider();
		...
		private void CheckForEyeData(object source, ElapsedEventArgs e)
		{
			...
			try {
				...
				if (extendedEyeGazeDataProvider.isActiveAndEnabled)
				{
					var combinedGazeReadingInWorldSpace = extendedEyeGazeDataProvider.GetWorldSpaceGazeReading(ExtendedEyeGazeDataProvider.GazeType.Combined, DateTime.UtcNow);
					//var date = new DateTime(1932, 10, 21, 23, 0, 0, DateTimeKind.Utc);
					var dt2 = new DateTimeOffset(DateTime.UtcNow);
					long unixTimeStamp = dt2.ToUnixTimeMilliseconds();

					if (combinedGazeReadingInWorldSpace.EyePosition != null)
					{
						dataQueue.Enqueue(new GazeAPIData()
						{
							EyeDataTimestamp = unixTimeStamp,
							EyeDataRelativeTimestamp = unixTimeStamp,
							IsCalibrationValid = true,
							GazeHasValue = true,
							GazeOrigin = combinedGazeReadingInWorldSpace.EyePosition,
							GazeDirection = combinedGazeReadingInWorldSpace.GazeDirection
						});
					}
					else
					{
